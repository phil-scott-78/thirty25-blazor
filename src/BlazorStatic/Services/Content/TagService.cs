...
/// <summary>
/// Service for managing and processing tags in a BlazorStatic site.
/// </summary>
    ...
    where TFrontMatter : class, IFrontMatter
{
    ...
    /// </summary>
    /// <param name="options">Content options containing tag configuration.</param>
    ...
    private Tag BuildTag(string tagName)
    {
        ...
        catch (Exception ex)
        {
            ...
            throw new BlazorStaticException($"Failed to build tag for '{tagName}'", ex);
        }
    }
    ...
    internal ImmutableList<Tag> ExtractTagsFromFrontMatter(TFrontMatter frontMatter)
    {
        ...
        try
        {
            if (frontMatter is IFrontMatterWithTags frontMatterWithTags)
            {
                // Filter out any empty or null tags before processing
                var validTags = frontMatterWithTags.Tags
                    .Where(tag => !string.IsNullOrWhiteSpace(tag))
                    .ToArray();

                _logger.LogDebug("Extracted {Count} tags from front matter", validTags.Length);
                return validTags.Select(BuildTag).ToImmutableList();
            }

            _logger.LogDebug("Front matter does not implement IFrontMatterWithTags");
            return [];
        }
    ...
    internal IEnumerable<Tag> GetUniqueTagsFromContentPages(IEnumerable<MarkdownContentPage<TFrontMatter>> contentPages)
    {
        ...
        try
        {
            ...
            _logger.LogDebug("Found {Count} unique tags across all content pages", uniqueTags.Count);
            return uniqueTags;
        }
        catch (Exception ex)
        {
